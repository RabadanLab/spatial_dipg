...
#making a data.frame including all thresholds 
...
###data transformation########################################################################################################################
##############################################################################################################################################
list.files(paste0(root, "data_transformed/")) #where all splitted TMA datas are stored
trans <- paste0(root, "data_transformed/")
trans_files <- list.files(trans)
#length(trans_files) #17 for TMA_272

#transform the data into binarized form by applying threshold
...
concat <- read.table(paste0(root, "concat_transformed.txt"), 
           sep="\t", header=F)
colnames(concat) <- c("name", "x", "y", thres_df$Ab)
write.table(concat, 
            paste0(root, "concat_transformed.txt"), 
...
##############################################################################################################################################
###below are codes to add a column specifying unique cell type per each row (each cell)
...
##############################################################################################################################################
###we will work on below data with uniqe cell type information
###for some codes, you need to call custom-functions at first (line 165)
...
#summary statistics for unique cell type
do.call(rbind,lapply(df_2[,c(2:7)], function(x){ summary_stat(x)}))

#unique cell types which exceeding predefined frequency of entire cells per TMA spot
...
abs <- colnames(data)[4:19] #16 antibodies used in this dataset
